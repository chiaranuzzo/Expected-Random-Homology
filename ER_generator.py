...
GER = nx.fast_gnp_random_graph(G.number_of_nodes(),
float(G.number_of_nodes())/float(G.number_of_edges()))

ww = nx.utils.powerlaw_sequence(GER.number_of_edges())
...
NW = new_weights(ww)
Ger_pwl=nx.Graph()
Ger_pwl.add_nodes_from(GER.nodes())
Ger_pwl.add_edges_from([(ed[0], ed[1], {'weight': w}) 
for ed,w in zip(GER.edges(), NW)])
...
for i in range(num_iter):
    G_rand = biased_random_generator(Ger_pwl)
    randi = open(dir+dataset_tag+'_ERpwl%d.pck' %i,'w')
...
betti_dict = {}
for i in range(num_iter):
    randi = open(dir+dataset_tag+'_ERpwl%d.pck' %i,'r')
    G_rand = pk.load(randi)
    nx.write_edgelist(G_rand, dir+dataset_tag+'_ERpwl%d.edges' %i)
    G_rand=nx.read_edgelist(dir+dataset_tag+'_ERpwl%d.edges' 
    ...
    fname=dir+dataset_tag+'_ERpwl%d_clique_filtration.pck' %i
    ...
    gname=dir+'gen/generators_'+dataset_tag+'_ERpwl%d.pck' %i
    gfile=open(gname,'w')
